...
const Tracking = () => {
  ...
  const getActiveShipmentDrivers = async () => {
    ...
    if (response.success) {
      ...
      if (data.length) {
        setShipments((state) => ({ ...state, all: data, selected: { shipmentId: data[0].shipmentId } }))
      }
    }
  }
  ...
  const selectShipment = (shipmentId) => {
    setShipments((state) => ({ ...state, selected: { shipmentId } }))
  }
  ...
  return (
    <div>
      ...
      <H3 className="my-5 text-gray-700" weight="normal">Lista de envíos activos</H3>
                ...
                <tr className="border-2 border-gray-100" key={data.shipmentId}>
                  ...
                  <td className="p-3.5">
                    <div className="flex gap-2">
                      ...
                      <button className="bg-blue-500 text-white text-sm rounded py-2 px-3" style={{ minWidth: "98.8px" }}>Detalle</button>
                    </div>
                  </td>
                </tr>
              )
